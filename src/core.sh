...
ask() {
    ...
    sni)
        ...
        2)
            load sni.sh
            # Call get_best_sni and capture output (last line)
            # get_best_sni prints messages to stderr/stdout mixed, 
            # so we should capture the result carefully or let it print and we just take the result.
            # My get_best_sni impl prints result to stdout at the end.
            is_best=$(get_best_sni | tail -n 1)
            export $is_ask_set=$is_best
            return
            ;;
        3)
            export $is_ask_set=$is_random_servername
            msg "使用随机: ${!is_ask_set}"
            return
            ;;
        *)
            # Manual input (default fallthrough)
            ask string $is_ask_set "请输入 SNI (serverName):"
            return
            ;;
        esac
        ;;
...
change() {
    ...
    10)
        ...
        [[ ! $is_new_servername ]] && ask sni is_new_servername "请选择 SNI (serverName) 配置方式:"
